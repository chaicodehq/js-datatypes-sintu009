...
export function buildZomatoOrder(cart, coupon) {
  ...
  const items = cart.filter(item => item.qty > 0).map(item => {
    const addonTotal = (item.addons || []).reduce((sum, addon) => sum + parseFloat(addon.split(':')[1]), 0);
  ...
  let discount = 0;
  const code = coupon ? coupon.toLowerCase() : '';
  if (code === 'first50') discount = Math.min(subtotal * 0.5, 150);
  else if (code === 'flat100') discount = 100;
  else if (code === 'freeship') { discount = deliveryFee; deliveryFee = 0; }
