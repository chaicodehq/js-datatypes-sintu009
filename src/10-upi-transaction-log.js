...
export function analyzeUPITransactions(transactions) {
  ...
  const totalCredit = valid.filter(t => t.type === 'credit').reduce((sum, t) => sum + t.amount, 0);
  const totalDebit = valid.filter(t => t.type === 'debit').reduce((sum, t) => sum + t.amount, 0);
  const netBalance = totalCredit - totalDebit;
  const transactionCount = valid.length;
  const avgTransaction = Math.round(valid.reduce((sum, t) => sum + t.amount, 0) / transactionCount);
  const highestTransaction = valid.reduce((max, t) => t.amount > max.amount ? t : max);
  const categoryBreakdown = valid.reduce((acc, t) => { acc[t.category] = (acc[t.category] || 0) + t.amount; return acc; }, {});
  const contactCounts = valid.reduce((acc, t) => { acc[t.to] = (acc[t.to] || 0) + 1; return acc; }, {});
  const frequentContact = Object.entries(contactCounts).reduce((max, curr) => curr[1] > max[1] ? curr : max)[0];
